$(()=>{
	var $chbAlls=$("div.check-top>img,div.all>span>img");
	...
	function loadCart(){
		$.getJSON("data/cart/getCart.php",items=>{
				...
				for(var item of items){
					var {iid,product_id,title,price,count,spec,is_checked,sm}=item;
					...
					html+=`<div class="imfor">
							...
							<div class="product">
							 <a href="product_details.html?lid=${product_id}" target="_blank">
								<img src="${sm}" alt="">
							 </a>
							 <span class="desc">
							  <a href="product_details.html?lid=${product_id}" target="_blank">${title}</a>
							 </span>
							 <p class="col">
							  <span>规格：</span>
	...
	$.getJSON("data/users/islogin.php",data=>{
		if(data.ok==1){
			loadCart();
			$("#content-box-body").on("click",".reduce,.add,.imfor>.check>img,.del>a",function(){
				var $tar=$(this);
				...
				if($tar.is("img")){
					...
					var n=$tar.siblings("input").val();
					if($tar.is(".add")){
						n++;
					}else{
						n--;
					}
					$tar.siblings("input").val(n);
					$.get("data/cart/updateCart.php",
						{
							iid:$tar.data("iid"),
							count:n
						},
