...
$(()=>{
	...
	$.getJSON("data/index/getIndexCarousel.php",data=>{
		...
		$ulIdxs.html("<li></li>".repeat(data.length)).children(":first").addClass("hover");
		$ulImgs.html(html).css("width",LIWIDTH*(data.length+1));
		function move(dir){
			dir=dir||1;
			if(moved==0&&dir==-1){
				moved=data.length;
			...
			$ulImgs.animate({
				...
				$ulIdxs.children(":eq("+moved+")").addClass("hover")
		...
		$("#banner").hover(
			...
			function(){timer=setInterval(move,wait);}
		);
		...
		$("a.ck-right").click(function(){
			if(!$ulImgs.is(":animated")){move();}
			
		});
		$("a.ck-left").click(function(){
			if(!$ulImgs.is(":animated")){move(-1);}
		});
	})
});
$(()=>{
	$.ajax({
		...
		var html="";
		var i=0;
		for(var b of top_sale){
			var {title,details,price,href,pic}=b;
			if(i<2){
				html+=`<div>
					<div class="desc">
						<p class="name">${title}</p>
