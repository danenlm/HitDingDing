...
//			data:"lid="+lid,
//			dataType:"json",
//			beforeSend:function(req){//发送之前调用
//				console.log("开始发送请求");
//			},
//			complete:function(req,data){//请求完成时
//				console.log("请求完成");
//				console.log(req);
//				console.log(data);
//			},
...
//		})
		...
		function success(data){
			...
			document.querySelector("#show-details>h3>a").innerHTML=subtitle;
			document.querySelector("#show-details>div.price>div.stu-price>span").innerHTML="¥"+parseFloat(price).toFixed(2);
			document.querySelector("#show-details>div.price>div.promise>b").innerHTML="服务承诺："+promise;
			var html="";
			for(var s of specs){
				var {lid,spec}=s;
				html+=`<a href="product_details.html?lid=${lid}" class=${lid===details.lid?"active":""}>${spec}</a>`;
			}
			document.querySelector("#show-details>div.spec>div").innerHTML=html;
			...
			document.getElementById("product-intro").innerHTML=details.details;
			...
			ulImgs.onmouseover=function(e){
				...
				if(tar.nodeName==="IMG"){
					...
					lgDiv.style.background="url("+tar.dataset.lg+")";
				}
			}
		...
		var $reduce=$("#show-details>.account>.number-reduce");
		$reduce.click(handler);
		var $add=$("#show-details>.account>.number-add");
		$add.click(handler);
		function handler(){
			//this->btn
			var $btn=$(this);
		...
		$("#show-details>.shops>.cart").click(function(){
			$.get("data/users/islogin.php",data=>{
				if(data.ok==1){
					var lid=location.search.split("=")[1];
					var count=$("#show-details>.account>input").val();
